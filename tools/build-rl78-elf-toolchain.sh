...
rundeps="sudo gmp mpfr mpc1 make libstdc++ nano git"
...
buildah config --workingdir "${workdir}/binutils-${binutils_ver}/build" $ctr
...
buildah config --workingdir "${workdir}/gcc-${gcc_ver}/build" $ctr
buildah run $ctr -- ../configure \
	--target=${target} \
	--prefix=${prefix} \
	--enable-languages=c \
...
buildah config --workingdir "${workdir}/gdb-${gdb_ver}/build" $ctr
buildah run $ctr -- ../configure \
	--target=${target} \
	--prefix=${prefix}
buildah run $ctr -- make -j${njobs}
